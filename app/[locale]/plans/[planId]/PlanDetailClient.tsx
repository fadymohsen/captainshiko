...
import { X, CreditCard, Wallet, Loader2, Receipt, CheckCircle2, Smartphone, Upload, ImageIcon, Star } from "lucide-react";
...
export function PlanDetailClient({ plan }: { plan: any }) {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.url && (paymentMethodId === 2 || paymentMethodId === 4 || paymentMethodId === 6)) {
              ...
              {instapayStep === "instructions" ? (
                ...
                <div className="text-center py-4">
                  ...
                  <a
                    href={`https://wa.me/201553038830?text=${encodeURIComponent(
                      locale === "en"
                        ? `Hi Coach Mohamed! I've just paid via ${paymentMethodId === 3 ? "Fawry" : "Wallet"} for the ${plan.nameEn} plan. Here is my receipt. (Invoice: ${paymentResponse.invoiceId || 'N/A'}, Ref: ${paymentResponse.paymentData?.fawryCode || paymentResponse.paymentData?.meezaReference || 'N/A'})`
                        : `أهلاً كوتش محمد! لسه دافع عن طريق ${paymentMethodId === 3 ? "فوري" : "المحفظة"} لباقة ${plan.nameAr}. ده إيصال الدفع. (رقم الفاتورة: ${paymentResponse.invoiceId || 'N/A'}, كود المرجع: ${paymentResponse.paymentData?.fawryCode || paymentResponse.paymentData?.meezaReference || 'N/A'})`
                    )}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-3 w-full py-5 rounded-full bg-[#25D366] text-white font-black hover:bg-[#20ba5a] transition-all shadow-lg shadow-green-500/20"
