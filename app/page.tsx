      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          <a href="#" className="text-xl font-extrabold tracking-tight">
            Captain Shiko<span className="text-muted">.</span>
          </a>
          ...
          <a
            href="#contact"
            className="border border-white/20 text-sm px-5 py-2 rounded-full hover:bg-white hover:text-background transition-all duration-300"
          >
            Sign Up
          </a>
        </div>
      ...
      <section className="relative min-h-screen flex items-center pt-20">
        <div className="max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-2 gap-8 items-center">
          ...
          <div className="relative flex items-center justify-center">
            {/* Vertical text */}
            <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 vertical-text text-[8rem] font-black text-white/[0.03] leading-none tracking-tight select-none hidden lg:block">
              FITNESS
            </div>
            {/* Photo placeholder */}
      ...
      <section id="about" className="py-28">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="flex flex-col gap-6">
            ...
            <a
              href="#contact"
              className="inline-flex w-fit bg-white text-background font-bold text-sm px-7 py-3.5 rounded-full hover:bg-white/90 transition-all"
            >
              View Detail
            </a>
          </div>
              ...
              <div
                ...
                className={`rounded-xl p-8 flex flex-col transition-all duration-300 ${
                  plan.highlighted
                    ...
                    : "bg-surface-light border border-border hover:border-white/15"
                }`}
              >
                <div className="mb-6">
                  <h3 className={`text-sm font-semibold mb-4 ${plan.highlighted ? "text-background/60" : "text-muted"}`}>
                    {plan.tier}
                  </h3>
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-extrabold">{plan.price}</span>
                    <span className={`text-sm ${plan.highlighted ? "text-background/50" : "text-muted"}`}>
                      {plan.period}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-3.5 mb-8 flex-1">
                  {plan.features.map((f) => (
                    <div key={f} className="flex items-start gap-3">
                      <div className={`w-1.5 h-1.5 rounded-full mt-2 shrink-0 ${plan.highlighted ? "bg-background" : "bg-white"}`} />
                      <span className={`text-sm ${plan.highlighted ? "text-background/70" : "text-muted"}`}>
                        {f}
                      </span>
                    </div>
                  ))}
          ...
          <div className="bg-surface-light rounded-2xl p-10 sm:p-16 flex flex-col lg:flex-row items-center gap-12 border border-border">
            ...
            <div className="flex flex-col gap-4 items-center lg:items-end">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="bg-white text-background font-bold text-sm px-8 py-3.5 rounded-full hover:bg-white/90 transition-all"
              >
                Get In Touch
              </a>
            </div>
